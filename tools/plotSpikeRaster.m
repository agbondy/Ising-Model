...
p.addParamValue('PlotType','vertline',@ischar);
...
p.addParamValue('VertSpikeHeight',0.7,@(x) isnumeric(x) && isscalar(x));
p.addParamValue('colorgroups',[]);
...
if isempty(colorgroups)
    colorgroups=ones(size(spikes,1),1);
end
...
if islogical(spikes)
    ...
    switch plotType
        ...
        case 'vertline'
            ...
            % Find the trial (yPoints) and timebin (xPoints) of each spike
            if ~isempty(colorgroups)
                groupids=unique(colorgroups);
                colors=distinguishable_colors(max(groupids));
                for i=groupids(:)'
                    badrows = colorgroups~=i;
                    currspikes=spikes;
                    currspikes(badrows,:)=0;
                    [trials,timebins] = find(currspikes);
                    trials = trials';
                    timebins = timebins';
                    ...
                    xPoints = xPoints(:);
                    yPoints = yPoints(:);
                    plot(xPoints,yPoints,lineFormat{:},'color',colors(i,:));hold on
                end
            end
        case 'horzline2'
